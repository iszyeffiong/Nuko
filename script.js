document.addEventListener('DOMContentLoaded', function() {
  ...
  // Start with hardcoded fallback addresses
  ...
  function loadAddressesFromCSV(csvUrl) {
    console.log("Loading addresses from CSV:", csvUrl);
    ...
    fetch(csvUrl)
      ...
      .then(csvText => {
        ...
        whitelistedWallets = [...new Set([...whitelistedWallets, ...addresses])];
        console.log("Total addresses in whitelist:", whitelistedWallets.length);
      })
      .catch(error => {
        console.error("Error loading CSV:", error);
        console.log("Using fallback addresses only");
      });
  }

  // Load addresses from CSV file (replace with your actual CSV URL)
  loadAddressesFromCSV('Whitelist.csv');
  
  // You could host the CSV on GitHub or directly in your project
  // For example: loadAddressesFromCSV('https://raw.githubusercontent.com/yourusername/yourrepo/main/whitelist.csv');
  ...
  checkButton.addEventListener('click', function(event) {
    ...
    const lowerCaseWhitelist = whitelistedWallets.map(addr => 
      typeof addr === 'string' ? addr.toLowerCase() : ''
    );
    
    console.log("Checking if address is in whitelist:", lowerCaseAddress);
    console.log("Contains address?", lowerCaseWhitelist.includes(lowerCaseAddress));
    
    // Case-insensitive check
    if (lowerCaseWhitelist.includes(lowerCaseAddress)) {
      console.log("Address found in whitelist (case-insensitive)");
